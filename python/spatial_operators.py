...
@njit
def get_modif_SUPG_developped_divFlux(q, iMin, iMax, jMin, jMax, dx, dy, coeffs):
...
@njit
def get_optim_modif_SUPG_developped_divFlux(q, iMin, iMax, jMin, jMax, coeffs):
    # allocation de variables pour chaque coeff
    # u equation - u term
    uEq_uTerm_xp1, uEq_uTerm_x, uEq_uTerm_xm1 = coeffs[0]
    uEq_uTerm_yp1, uEq_uTerm_y, uEq_uTerm_ym1 = coeffs[1]
...
@njit
def get_optim_modif_SUPG_developped_divFlux_AND_physical_Terms(q, iMin, iMax, jMin, jMax, coeffs):
    # allocation de variables pour chaque coeff
    # u equation - u term
    uEq_uTerm_xp1, uEq_uTerm_x, uEq_uTerm_xm1 = coeffs[0]
    uEq_uTerm_yp1, uEq_uTerm_y, uEq_uTerm_ym1 = coeffs[1]
    ...
    divF, physicalTerms = np.zeros((2, ) + (np.shape(q)))
    ...
    for i in range(iMin, iMax+2):
        for j in range(jMin, jMax+2):
            ...
            uEq_pTerm_ij = uEq_pTerm_xp1 * uEq_pTerm_yOp[i+1, j] + uEq_pTerm_x * uEq_pTerm_yOp[i, j] + uEq_pTerm_xm1 * uEq_pTerm_yOp[i-1, j]
            divF[i, j, XVEL] = 0.5 * uEq_uTerm_ij + 0.5 * uEq_vTerm_ij + uEq_pTerm_ij
            physicalTerms[i, j, XVEL] = uEq_pTerm_ij     # dx_p
            ...
            vEq_pTerm_ij = vEq_pTerm_xp1 * vEq_pTerm_yOp[i+1, j] + vEq_pTerm_x * vEq_pTerm_yOp[i, j] + vEq_pTerm_xm1 * vEq_pTerm_yOp[i-1, j]
            divF[i, j, YVEL] = 0.5 * vEq_uTerm_ij + 0.5 * vEq_vTerm_ij + vEq_pTerm_ij
            physicalTerms[i, j, YVEL] = vEq_pTerm_ij     # dy_p
            ...
            pEq_pTerm2_ij = pEq_pTerm2_xp1 * pEq_pTerm2_yOp[i+1, j] + pEq_pTerm2_x * pEq_pTerm2_yOp[i, j] + pEq_pTerm2_xm1 * pEq_pTerm2_yOp[i-1, j]
            divF[i, j, PRES] = pEq_uTerm_ij + pEq_vTerm_ij + 0.5 * pEq_pTerm1_ij + 0.5 * pEq_pTerm2_ij
            physicalTerms[i, j, PRES] = pEq_uTerm_ij + pEq_vTerm_ij     # div U
...
@njit
def get_optim_modif_SUPG_physical_Terms(q, iMin, iMax, jMin, jMax, coeffs):
    # allocation de variables pour chaque coeff
    # u equation - p term
    uEq_pTerm_xp1, uEq_pTerm_x, uEq_pTerm_xm1 = coeffs[4]
    uEq_pTerm_yp1, uEq_pTerm_y, uEq_pTerm_ym1 = coeffs[5]
    ...
    physicalTerms = np.zeros((np.shape(q)))
...
def getApproxDivFlux(q, grid, iMin, iMax, jMin, jMax, dx, dy, schemeChoice, operatorsCoeff):
    if schemeChoice == 1 :
        return get_UPWIND_divFlux(q, grid)
    if schemeChoice == 2 :
        return get_SUPG_developped_divFlux(q, grid)
    if schemeChoice == 3 :
        return get_modif_SUPG_developped_divFlux(q, iMin, iMax, jMin, jMax, dx, dy, operatorsCoeff)
    if schemeChoice == 4 :
        return get_optim_modif_SUPG_developped_divFlux(q, iMin, iMax, jMin, jMax, operatorsCoeff)
    if schemeChoice == -1 :
        # return get_SUPG_developped_divFlux(q, grid)
        return get_modif_SUPG_developped_divFlux(q, iMin, iMax, jMin, jMax, dx, dy, operatorsCoeff)
        # return get_optim_modif_SUPG_developped_divFlux(q, iMin, iMax, jMin, jMax, operatorsCoeff)
...
def getApproxDivFluxAndPhysicalTerms(q, grid, iMin, iMax, jMin, jMax, dx, dy, schemeChoice, operatorsCoeff):
    if schemeChoice == 4 :
        return get_optim_modif_SUPG_developped_divFlux_AND_physical_Terms(q, iMin, iMax, jMin, jMax, operatorsCoeff)
